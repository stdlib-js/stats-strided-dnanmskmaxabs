...
var dnanmskmaxabs = tryRequire( resolve( __dirname, './../lib/ndarray.native.js' ) );
var opts = {
	'skip': ( dnanmskmaxabs instanceof Error )
};
...
tape( 'main export is a function', opts, function test( t ) {
	...
	t.strictEqual( typeof dnanmskmaxabs, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function has an arity of 7', opts, function test( t ) {
	t.strictEqual( dnanmskmaxabs.length, 7, 'has expected arity' );
	t.end();
});

tape( 'the function calculates the maximum absolute value of a strided array according to a mask, ignoring NaN values', opts, function test( t ) {
	var mask;
	var x;
	var v;
	...
	v = dnanmskmaxabs( x.length, x, 1, 0, mask, 1, 0 );
...
tape( 'if provided an `N` parameter less than or equal to `0`, the function returns `NaN`', opts, function test( t ) {
	...
	v = dnanmskmaxabs( 0, x, 1, 0, mask, 1, 0 );
	t.strictEqual( isnan( v ), true, 'returns expected value' );

	v = dnanmskmaxabs( -1, x, 1, 0, mask, 1, 0 );
	t.strictEqual( isnan( v ), true, 'returns expected value' );
...
tape( 'the function supports `stride` parameters', opts, function test( t ) {
	...
	v = dnanmskmaxabs( 6, x, 2, 0, mask, 2, 0 );
...
tape( 'the function supports negative `stride` parameters', opts, function test( t ) {
	...
	v = dnanmskmaxabs( 6, x, -2, 10, mask, -2, 10 );
...
tape( 'the function supports `offset` parameters', opts, function test( t ) {
	...
	v = dnanmskmaxabs( 6, x, 2, 1, mask, 2, 1 );
	t.strictEqual( v, 4.0, 'returns expected value' );
